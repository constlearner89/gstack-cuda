...
This fork also includes a simulation-oriented extension layer that applies gstack-style workflows to CUDA/C++ numerical software.

| Skill | Your specialist | What they do |
|-------|----------------|--------------|
| `/cuda-office-hours` | **Simulation Research Lead** | Reframes solver or model ideas into the narrowest high-value wedge before coding. |
| `/cuda-plan-eng-review` | **Simulation Tech Lead** | Reviews architecture, discretization risk, data layout, GPU execution, verification, and benchmark plans. |
| `/cuda-dev` | **CUDA Engineer** | Default implementation workflow for solver development. |
| `/cuda-investigate` | **GPU Debugger** | Root-cause investigation for crashes, NaNs, instability, and CPU/GPU mismatches. |
...
| `/cuda-benchmark` | **Performance Engineer** | Performance validation for kernels and representative workloads. |
| `/cuda-verify` | **Verification Engineer** | Evidence-driven verification across deterministic cases, regressions, and analytical references. |
| `/cuda-release` | **Simulation Release Engineer** | Release discipline for solver repositories with verification and benchmark gates. |

Use these when the primary task is solver development rather than web product work.
