...
description: |
  ...
  - Brainstorming a solver/model idea (DEM/SPH/FVM/FEM/CUDA) → suggest /cuda-office-hours
  - Reviewing a solver implementation plan → suggest /cuda-plan-eng-review
  - Implementing CUDA/C++ numerical code → suggest /cuda-dev
  - Debugging NaNs, divergence, or GPU mismatches → suggest /cuda-investigate
  - Reviewing solver or kernel diffs → suggest /cuda-review
  - Measuring kernel or solver speed → suggest /cuda-benchmark
  - Verifying numerical correctness → suggest /cuda-verify
  - Releasing solver changes with evidence → suggest /cuda-release
